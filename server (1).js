...
// blocks the frontend (a different origin on InfinityFree) from loading
// audio/cover images served by this API — causes
// ERR_BLOCKED_BY_RESPONSE.NotSameOrigin in the browser. Since this API's
// whole purpose is serving media to a separate frontend, disable it here
// and set the permissive version explicitly per-route instead.
...
// ---------- Announcements ----------
// Same persistence pattern as track metadata: local file + best-effort
// R2 mirror, restored from R2 on boot so a Railway redeploy doesn't
// wipe announcements either.
const ANNOUNCEMENTS_FILE = path.join(uploadDir, "announcements.json");
const ANNOUNCEMENTS_R2_KEY = "_announcements.json";
...
const saveAnnouncements = (list) => {
  ...
  if (s3) {
    s3.send(new PutObjectCommand({
      Bucket: R2_BUCKET,
      Key: ANNOUNCEMENTS_R2_KEY,
      Body: JSON.stringify(list, null, 2),
      ContentType: "application/json"
    })).catch(e => console.warn("announcements R2 mirror failed:", e.message));
  }
};

async function restoreAnnouncementsFromR2() {
  if (!s3) return;
  try {
    const cmd = new GetObjectCommand({ Bucket: R2_BUCKET, Key: ANNOUNCEMENTS_R2_KEY });
    const res = await s3.send(cmd);
    const body = await res.Body.transformToString();
    fs.writeFileSync(ANNOUNCEMENTS_FILE, body);
    console.log("Restored announcements.json from R2");
  } catch (e) {
    console.log("No existing announcements.json in R2 (fine on first run)");
  }
}
await restoreAnnouncementsFromR2();
...
// ---------- Announcements ----------
// Public read (frontend shows a banner to all visitors), admin-only write.
...
// ---------- Admin stats ----------
// Storage usage and library counts for the admin dashboard. Walks the
// full bucket listing (already paginated via listAllObjects) rather
// than tracking a running counter, so it's always accurate even if
// files were added directly via rclone rather than through /upload.
...
app.get("/admin/stats", requireAdmin, async (req, res) => {
  try {
    ...
    const meta = loadMeta();
    const { tracks, covers } = groupByAlbumFolder(allObjects);
    ...
    for (const t of tracks) {
      const m = meta[t.Key] || {};
      const folderName = t._folderPath ? t._folderPath.split("/").pop() : null;
      albumNames.add(m.albumName || folderName || "Unknown Album");
      artistNames.add(m.artistName || "Unknown");
    }
    ...
    res.json({
      ...
      coverCount: Object.keys(covers).length,
      totalObjects: allObjects.length,
      totalBytes,
      totalMB: Math.round((totalBytes / 1024 / 1024) * 10) / 10,
      announcementCount: loadAnnouncements().length,
...
app.get("/drive/:id", async (req, res) => {
  try {
    ...
    // Without this, browsers block <audio>/<video> from loading the
    // response cross-origin with ERR_BLOCKED_BY_RESPONSE.NotSameOrigin,
    // even though CORS headers are set — CORP is a separate mechanism.
    res.setHeader("Cross-Origin-Resource-Policy", "cross-origin");
